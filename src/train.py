...
def fit(model,
    ...
    Args:
        ...
        optimizer ([type]): the optimizer to use
        loss ([type]): the objective function for training
        epochs (int): number of epochs to train for
    """
    size = len(train_dataloader)
    for epoch in range(num_epochs):
        print(f"Epoch {epoch}")
        for batch, (data, labels) in enumerate(track(train_dataloader, 
...
def main():
    ...
    train_data, val_data = data.make_dataloaders(*data.load_mnist(),
                                                 batch_size=16)

    cnn = model.get_model()
    optimizer=model.get_optimizer(params["optimizer"],
                                  nn.ParameterList(cnn.parameters()),
